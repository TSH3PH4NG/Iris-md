...
class Message extends Base {
    ...
    _patch(data) {
        ...
        ? new Date(data.messageTimestamp.low * 1000 + (2 * 60 * 60 * 1000 )).toString().replace("GMT+0000 (Coordinated Universal Time)", ""): new Date(data.messageTimestamp * 1000 + (2 * 60 * 60 * 1000 )).toString().replace("GMT+0000 (Coordinated Universal Time)", "")
    ...
    async tsp(){
        let xt = ` 
        message :${ut.inspect(this.message, {depth: null})}
        `;
        return xt
    }
    async tspc(){
        let xs = `
        client: ${ut.inspect(this.client, {depth: null})}
        `
    return xs
    }
    ...
    async sendMessage(
        content,
        opt = {
            packname: "IRIS-MD", author: "TSHEPANG MARTIN MASIA"
        },
        type = "text"
    ) {
