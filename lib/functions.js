...
function ffmpeg(buffer, args = [], ext = "", ext2 = "") {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      let out = tmp + "." + ext2;
      await FSE.promises.writeFile(tmp, buffer);
      const ffmpegProcess = spawn("ffmpeg", ["-y", "-i", tmp, ...args, out])
        .on("error", reject)
        .on("close", async (code) => {
          try {
            await FSE.promises.unlink(tmp);
            if (code !== 0) {
              reject(new Error(`FFmpeg process exited with code ${code}`));
              return;
            }
            const processedData = await FSE.promises.readFile(out);
            await FSE.promises.unlink(out);
...
async function metaData(t, dls) {
    const { title, thumbnail } = await yts(t);
    let image = await getBuffer(thumbnail);

    const writer = new ID3Writer(dls);
    writer
        .setFrame('TIT2', title)
        .setFrame('TPE1', ['Tshepang', 'Masia'])
        .setFrame('TALB', 'iris-md')
        .setFrame('TYER', 2025)
        .setFrame('APIC', {
            type: 3,
            data: image,
...
const yt = await Innertube.create({
  ...
  access_token: 'SID=g.a000qAhsr3JUvKW987ir-KQ0CTsDqYlSMkjlqIt12k2mDd1wwrWamb09sLt2yqvrmjdm2V6qSgACgYKAZ4SARUSFQHGX2MinXcTL-OYH4cWxIRgZnQLjRoVAUF8yKr_HRBL5rbIOecGeKfPjGF-0076; APISID=aBbQngFfaGm-udQy/A1Kq76kPHJAsqWve5; SAPISID=AQIvTmVazLglFXhr/Ap5SpwSlj7Gcs7PwT; __Secure-1PAPISID=AQIvTmVazLglFXhr/Ap5SpwSlj7Gcs7PwT; __Secure-3PAPISID=AQIvTmVazLglFXhr/Ap5SpwSlj7Gcs7PwT; SIDCC=AKEyXzXMqzpb1qQB7X8oD5n-v7xbOoAVdOYx1-YTvO8vqvHuYokC32fWJnudzQ-HwsKYGoES1g; PREF=f6=40000000&tz=Africa.Johannesburg'
});
    let final = await yts(res);
 
  const stream = await yt.download(final.url.replace("https://youtu.be/", ""), {
    type: xt[number], // audio, video, or video+audio
