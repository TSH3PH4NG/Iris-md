...
const {title , url , bestThumbnail , id } = await syt(t);
let image = await getBuffer(bestThumbnail.url)
...
const tags = {
  ...
  const success = await NodeID3.write(tags, dls)
  return success;
};
...
let sPath = "./song.m4a";
const file = await createWriteStream(sPath);
...
let bb = await readFileSync(sPath);
return bb
